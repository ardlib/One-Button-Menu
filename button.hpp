...
typedef void (*CB)(bool state, uint32_t since);
...
public:
  ...
  OBM() : OBM(255, 255, NULL, 0) {}

  // Direct Specification type Constructor
  OBM(byte pin, uint8_t mode = INPUT_PULLUP) : OBM(pin, mode, NULL, 0) {}
    ...
    if (isPressed) {
      ...
      if (ts != 0 && ((ct - ts) >= debounce_ms)) {
        ...
        if (state) {
          ...
          return true;
          // End of Pressed State Detection
        } else {
          // Button was released prematurely
          isPressed = false;
...
#endif
        } // End of Level Detection for Button

      } // End of Time check for button
